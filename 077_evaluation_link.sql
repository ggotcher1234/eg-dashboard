-- 077_evaluation_link.sql
--
-- Depends on 075_document_sort_order.sql (latest get_client_public_view /
-- finalize_client), 037_client_logo_upload.sql (public-bucket + RLS pattern
-- this copies).
--
-- Greg (8/25/26): the close-out evaluation.html tool (076's cc_emails
-- feature and the earlier ratings-summary/jobs-question cleanup were
-- unrelated small fixes to that same file) needs to stop being a file Greg
-- keeps on his own machine and re-emails per client. Every Team Lead should
-- be able to generate a client's evaluation link from inside client_profile.
-- html, and that link needs to actually show up on the client's own
-- dashboard (client_public.html) -- both while the engagement is still
-- live ("Preview Dashboard") and afterward in the locked "Final Engagement
-- Archive" snapshot.
--
-- Three parts:
--   1. client_content.evaluation_link_url -- one more workflow-style URL
--      column, same shape as the four existing ones (discovery_doc_url
--      etc.), holding the public link to THIS client's baked evaluation
--      form (not the completed/returned survey -- that's still
--      closeout_survey_url/storage_path, unchanged).
--   2. A new public storage bucket, client-evaluations, so the generated
--      per-client evaluation.html can be opened by the CEO with no login
--      (same reasoning as 037's client-logos bucket: this isn't sensitive
--      client data, it's a blank form with that client's own research
--      questions baked in, meant to be opened by an outsider). Write is
--      gated to Super Admin / anyone assigned to the client; read is public
--      like every other bucket in this family.
--   3. get_client_public_view() and finalize_client() both get
--      evaluation_link_url added into the `workflow` jsonb they already
--      build, so the link flows into both the live view and the archived
--      snapshot without any other shape change. Bodies below are the full,
--      current 075 versions with only that one key added -- everything
--      else is unchanged.
--
-- Safe to re-run.

-- ---------- 1. client_content column ----------
alter table client_content add column if not exists evaluation_link_url text;

-- ---------- 2. client-evaluations public bucket ----------
insert into storage.buckets (id, name, public)
values ('client-evaluations', 'client-evaluations', true)
on conflict (id) do update set public = true;

-- Path convention: clients/{client_id}/evaluation.html (one file per
-- client, overwritten on regenerate -- same "one current version" model as
-- the client-logos bucket).
drop policy if exists "client evaluations write" on storage.objects;
create policy "client evaluations write" on storage.objects for all
using (
  bucket_id = 'client-evaluations'
...
drop policy if exists "client evaluations read" on storage.objects;
create policy "client evaluations read" on storage.objects for select
using (bucket_id = 'client-evaluations');
