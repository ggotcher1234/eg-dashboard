...
--
-- Greg's reference tool has 4 columns on Competitors/Top Customers:
-- Company, URL, Location, Notes. Ours only had 3 (name, website_url,
-- notes) -- Location was missing entirely. Adding it now, ahead of CSV
-- import/export (which needs a real column to map "Location" onto).
--
-- Safe to re-run.

alter table client_competitors add column if not exists location text;
alter table client_top_customers add column if not exists location text;

-- ----------------------------------------------------------------------
-- get_client_public_view: identical to 027's version except the
-- competitors/top_customers jsonb_build_object calls gain 'location'.
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- finalize_client: identical to 027's version except the
-- competitors/top_customers jsonb_build_object calls gain 'location'.
-- ----------------------------------------------------------------------
