-- 035_resource_vault_links.sql
--
-- Depends on 027_resource_vault_templates.sql.
--
-- Greg: "some of the resources are links and some are uploads." Right now
-- every Resource Vault entry -- shared template or client-specific -- can
-- only be an uploaded file in the UI. client_resource_vault_items has had a
-- nullable `url` column since day one (019_client_profile_content.sql) --
-- it just was never exposed in the Client Profile screen. resource_vault_
-- templates never had a url column at all. This migration adds it there
-- too and wires both public-view functions to resolve it the same way
-- title/description/storage_path already do (the client's own row wins if
-- set, otherwise it falls back to the linked template's).
--
-- client_public.html already knows how to render either kind (storage_path
-- -> signed-URL download link, url -> plain external link, in that
-- priority) -- that part shipped in 027, it just never had real template
-- URLs to resolve. No public.html changes needed.
--
-- Safe to re-run.

alter table resource_vault_templates add column if not exists url text;

-- ----------------------------------------------------------------------
-- get_client_public_view: identical to 027's version except the
-- resource_vault jsonb_build_object also resolves `url` from the linked
-- template (coalesce(r.url, rvt.url)), same pattern as the other three
-- fields.
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- finalize_client: identical to 027's version, same coalesce(r.url, rvt.url)
-- addition so a closed-out engagement's frozen snapshot also carries the
-- resolved link.
-- ----------------------------------------------------------------------
