-- 032_workflow_document_uploads.sql
--
-- Depends on 030_anon_read_client_documents.sql and
-- 031_competitors_location_field.sql.
--
-- Greg: "the EG Workflow Documents are client unique files that the Team
-- Lead needs to upload into this section." Confirmed via follow-up:
-- Discovery Document, Controlling Document, and Close-Out Survey become
-- file uploads (Super-Admin/Team-Lead uploads the actual file, same as
-- Resource Vault's custom items). Research Review stays a pasted URL --
-- it's explicitly a link to an externally-hosted Fathom call recording,
-- not a document to upload.
--
-- Safe to re-run.

alter table client_content add column if not exists discovery_doc_storage_path text;
alter table client_content add column if not exists controlling_doc_storage_path text;
alter table client_content add column if not exists closeout_survey_storage_path text;

-- research_review_url is untouched -- stays URL-only.

-- ----------------------------------------------------------------------
-- client_public_document_readable (030): extend the anon-read check to
-- also cover a client's uploaded workflow documents, the same way it
-- already covers documents.storage_path and
-- client_resource_vault_items.storage_path. Files live under
-- clients/{id}/workflow/... (same "clients/..." storage prefix as
-- everything else), gated the same way -- only for a client whose share
...
as $$
  ...
  or exists (
    ...
    join client_share_links l on l.client_id = cc.client_id and l.active
    where p_storage_path in (cc.discovery_doc_storage_path, cc.controlling_doc_storage_path, cc.closeout_survey_storage_path)
  );
$$;
...
-- ----------------------------------------------------------------------
-- get_client_public_view: identical to 031's version except the
-- `workflow` jsonb_build_object gains the 3 new storage_path fields.
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- finalize_client: identical to 031's version except the `workflow`
-- jsonb_build_object gains the 3 new storage_path fields.
-- ----------------------------------------------------------------------
