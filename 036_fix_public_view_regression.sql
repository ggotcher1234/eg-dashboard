-- 036_fix_public_view_regression.sql
--
-- Depends on 035_resource_vault_links.sql.
--
-- My mistake: 035 rebuilt get_client_public_view() and finalize_client()
-- starting from 027's version of those functions (to add the resource
-- vault `url` coalesce) and didn't carry forward everything three later
-- migrations had already added on top of 027:
--   032 -- workflow doc storage_path fields (discovery/controlling/
--         closeout uploads). Without these, an uploaded Discovery/
--         Controlling/Close-Out document stops showing as "filled" on the
--         client dashboard's workflow tracker -- this is the bug Greg hit.
--   033/034 -- research_areas restructured as questions with nested
--         documents (instead of a flat per-area file list), plus hiding
--         blank auto-created questions.
--   031 -- `location` field on competitors/top_customers.
--
-- This migration restores the full 034 structure and re-adds only the
-- actual new thing from 035 (coalesce(r.url, rvt.url) in resource_vault).
-- Nothing here is new schema -- it's a function-only fix.
--
-- Safe to re-run.
