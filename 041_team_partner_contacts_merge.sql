-- 041_team_partner_contacts_merge.sql
--
-- Depends on 040_client_team_partner_contacts.sql.
--
-- Third piece of the partner-contacts feature. Greg confirmed: partner-org
-- contacts should show in ONE combined "Economic Gardening Team" list
-- alongside internal EG staff, same name/title/email/phone shape, no visual
-- separation needed.
--
-- Identical to 038_team_contact_details.sql's version of both
-- get_client_public_view() and finalize_client() except the `team` value is
-- now the EG-staff coalesce block CONCATENATED (||) with a second coalesce
-- block pulling the client's selected partner contacts
-- (client_team_partner_contacts -> econ_dev_partner_contacts). Every other
-- field -- company, workflow storage paths, nested research questions,
-- documents, contacts, competitors/top_customers with location, resource
-- vault url coalesce -- is carried forward unchanged from 038.
--
-- Partner contacts don't have a specialty_type enum value, so their `title`
-- (e.g. "Program Administrator", "Executive Director") is put straight into
-- the same `specialty` jsonb key EG staff use. client_public.html already
-- falls back to the raw string when a specialty isn't a known code
-- (`SPECIALTY_LABELS[r.specialty] || r.specialty`), so no page changes are
-- needed for the label to render correctly.
--
-- Only active partner contacts (econ_dev_partner_contacts.active) are
-- included, ordered by client_team_partner_contacts.sort_order, and they're
-- appended after EG staff.
--
-- Safe to re-run.
