...
-- ---------------------------------------------------------------
-- 3. Backfill the engagements that already exist.
--
-- Only rows nobody has claimed. If a real person was put on an Admin or QC
-- row deliberately, they stay.
-- ---------------------------------------------------------------
do $$
declare
  v_house uuid;
...
begin
  ...
  if v_house is null then
    raise notice 'No house account flagged yet -- run Step 4 below, then re-run this file.';
    return;
  end if;
  ...
  raise notice 'Backfilled % Admin/QC row(s) to the house account.', n;
end $$;

-- ---------------------------------------------------------------
-- 4. ONE-TIME: point the flag at the NCEG Admin account.
--
-- Create it first from Roster -> Add Roster Member:
--   Name  NCEG Admin        Role  Specialist
--   Email admin@economicgardening.org   (it never signs in)
--   then tick "Hide from Roster" on its profile.
--
-- Then uncomment and run this, and re-run Step 3 above.
-- ---------------------------------------------------------------
-- update users
