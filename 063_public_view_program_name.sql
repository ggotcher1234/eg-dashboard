-- 063_public_view_program_name.sql
--
-- Depends on 036_fix_public_view_regression.sql.
--
-- Greg (8/21/26) asked for the client_public.html footer to show the
-- funding Program name (e.g. the EG Program/partner an engagement is
-- attached to), alongside the existing Dashboard/Confidential columns.
-- That data (econ_dev_companies.name via clients.econ_dev_company_id)
-- wasn't part of the "company" jsonb bundle returned by
-- get_client_public_view() or stored by finalize_client(), so this adds
-- a `program_name` key to that bundle in both places -- null if the
-- client isn't linked to a Program (client_profile.html already treats
-- this as a normal, handled case).
--
-- This is a function-only change (no new columns/tables) -- restores the
-- full 036 body verbatim and adds one left join + one jsonb key in each
-- of the two `company` object builds.
--
-- Safe to re-run.
