-- 038_team_contact_details.sql
--
-- Depends on 036_fix_public_view_regression.sql.
--
-- Greg: "the EG Contacts need to show just like the client contacts with
-- name, title, email and phone." The `team` bundle only ever carried name
-- + specialty -- email/phone already exist on `users` (same columns Team
-- Directory uses), just weren't included here. Identical to 036's version
-- of both functions except the `team` jsonb_build_object gains 'email' and
-- 'phone'.
--
-- Safe to re-run.
