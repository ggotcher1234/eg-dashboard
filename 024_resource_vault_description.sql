-- 024_resource_vault_description.sql
--
-- Depends on 022_brand_header_fields.sql.
--
-- The reference tool's Resource Vault has three fields per item: Title,
-- Description ("shows below icon" -- i.e. it's meant to be client-facing),
-- and URL/Path. We only had title + url. Adds the missing description
-- column and threads it through both places the resource_vault jsonb
-- bundle is built: get_client_public_view() (live branch) and
-- finalize_client() (snapshot branch).
--
-- Safe to re-run.

alter table client_resource_vault_items add column if not exists description text;

-- ----------------------------------------------------------------------
-- get_client_public_view: identical to 022's version except the
-- resource_vault jsonb_build_object gains 'description'.
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- finalize_client: identical to 022's version except the resource_vault
-- jsonb_build_object gains 'description'.
-- ----------------------------------------------------------------------
