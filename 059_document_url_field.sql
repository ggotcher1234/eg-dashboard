-- 059_document_url_field.sql
--
-- Depends on 057_workflow_docs_upload_or_link.sql (get_client_public_view /
-- finalize_client).
--
-- Bug found live (Greg, 8/19/26): the Clippard Instrument Labs client
-- dashboard was hanging on "Loading..." forever, but only for clients with
-- real attachments -- clients with no files/links attached loaded fine.
--
-- Root cause: `documents` gained a `url` column a while back so a research
-- question's attached "document" can be either an uploaded file
-- (storage_path) or a plain link (url) -- but get_client_public_view() and
-- finalize_client() never actually SELECT d.url when building the
-- per-question / per-area / general documents JSON. So a link-type
-- document (Clippard has one named "Clippard Listening Post") came back
-- from the database with neither a usable url nor a storage_path, which
-- crashed the page's rendering code (client_public.html has a matching
-- defensive fix so this can never hang the page again either way, but this
-- migration is the real, permanent fix -- it's why that link wasn't
-- opening/showing correctly even before it started crashing).
--
-- This migration -- identical to 057's versions of both functions, except
-- every 'documents' jsonb_build_object() call (per-question docs, legacy
-- pre-Questions docs, and general/unfiled docs -- 3 spots per function, 6
-- total) now also includes 'url', d.url (or d2.url). Every other field
-- carried forward unchanged from 057.
--
-- Safe to re-run.

-- ----------------------------------------------------------------------
-- get_client_public_view() / finalize_client()
-- ----------------------------------------------------------------------
