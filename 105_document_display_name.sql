-- 105_document_display_name.sql
--
-- Depends on 104_public_view_controlling_doc_link.sql (the current
-- get_client_public_view() / finalize_client() bodies this copies).
--
-- Greg (9/1/26): "the ability to name a file instead of using its file
-- name... include a Zip file, name it Competitor KW Files, click the icon
-- to download." Research-area files on the client dashboard showed their
-- raw upload filename; links already had a label at creation but uploaded
-- files never did.
--
-- documents.display_name is a nullable human label. Every UI now shows
-- display_name || file_name; the real file_name / storage_path / signed-URL
-- download are untouched. Set inline on the Research page
-- (client_research.html) via a rename control + an optional prompt at
-- upload; shown on the client dashboard (client_public.html) with the
-- existing file-type icon.
--
-- Bodies below are the full, current 104 versions of get_client_public_view()
-- and finalize_client() with 'display_name' added to every research-area
-- document object (per-question docs, the unsorted bucket, and the loose
-- research_area-is-null list). Everything else is unchanged.
--
-- Already-finalized engagements' snapshots won't carry display_name (only
-- new finalizations rebuild the research jsonb) -- acceptable.
--
-- Safe to re-run.
              ...
              coalesce((
                ...
                from (
                  ...
                  select 2147483647 as entry_sort,
                    jsonb_build_object(
                      'question', null,
                      'documents', jsonb_agg(jsonb_build_object('file_name', d2.file_name, 'display_name', d2.display_name, 'storage_path', d2.storage_path, 'url', d2.url) order by d2.sort_order nulls last, d2.uploaded_at desc)
                    ) as entry
                  from documents d2
