-- 104_public_view_controlling_doc_link.sql
--
-- Depends on 080_evaluation_response_detail.sql (the current
-- get_client_public_view() / finalize_client() bodies this copies) and
-- 103_controlling_doc_client_response.sql (client_content.controlling_doc_link_url).
--
-- Greg (9/1/26): "make the client dashboard showcase the link that we send
-- to the client -- that version of the CD is much nicer." client_public.html
-- gets everything through the workflow jsonb; this exposes
-- controlling_doc_link_url there (the deterministic
-- controlling_document_public.html?client=<id> URL saved by
-- client_workflow_files.html's Create Email flow) so the "Controlling
-- Document" step on the client dashboard can point at the polished
-- client-facing review page instead of the internal editor.
--
-- Bodies below are the full, current 080 versions of get_client_public_view()
-- and finalize_client() with one key added -- 'controlling_doc_link_url' --
-- to the `workflow` jsonb each builds. Everything else is unchanged.
--
-- Safe to re-run.
