-- 057_workflow_docs_upload_or_link.sql
--
-- Depends on 045_top_business_issues.sql (get_client_public_view / finalize_client).
--
-- Greg (8/18/26), looking at the Dashboard Wizard's EG Workflow Documents
-- step: "these documents can all be uploads or links. remove fathom text as
-- it is particular to my tool set." Discovery/Controlling/Close-Out already
-- supported both an uploaded file (*_storage_path) and a link (*_url) at
-- the database level -- the wizard UI just never exposed the link option.
-- Research Review was the odd one out: URL-only, with placeholder copy
-- hardcoded to Fathom (Greg's own call-recording tool, not something every
-- EG specialist necessarily uses). This migration closes that last gap:
--
--   1. client_content gains research_review_storage_path, so Research
--      Review can be an upload too, same shape as the other three.
--   2. get_client_public_view() / finalize_client() -- identical to 045's
--      versions except the `workflow` jsonb_build_object also resolves
--      'research_review_storage_path'. Every other field carried forward
--      unchanged from 045.
--
-- (The actual "remove Fathom wording" + "let Research Review be uploaded"
-- UI work happens in client_profile.html/client_public.html, not here.)
--
-- Safe to re-run.

alter table client_content add column if not exists research_review_storage_path text;
